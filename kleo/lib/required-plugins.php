...
function kleo_get_required_plugins() {
	...
	$required_plugins = array(
		...
		array(
			'name'               => 'Youzer - Extend BuddyPress User Profile',
			// The plugin name
			'slug'               => 'youzer',
			// The plugin slug (typically the folder name)
			'required'           => false,
			// If false, the plugin is only 'recommended' instead of required
			'version'            => kleo_get_plugin_version( 'youzer' ),
			// E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
			'source'             => kleo_get_plugin_src( 'youzer' ),
			// The plugin source
			'force_activation'   => false,
			// If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
			'force_deactivation' => false,
...
function kleo_get_plugin_src( $name ) {
	...
	return 'https://updates.seventhqueen.com/check/?action=download&slug=' . $name . '.zip';
